...
public class AsteroidSkin extends Skin {
    ...
    public AsteroidSkin(WorldRenderer renderer, final CelestialObject object) {
        ...
        elements.add(new V3DColorElement(new V3DShaderElement(elementStructure, "propeller"), new I3dColor(227, 205, 182)));
    ...
    public void update(Timestamp time) {
        ...
        if(lastDustEmission.durationTo(time.getGameTime()).longer(new Duration(0.5f))) {
            lastDustEmission = Time.getGameTime();
            renderer.addElement(new AsteroidDust(renderer, transform.getTranslation(), new Vec3(object.getFirstPart().getShape()).multiply(0.2) , new I3dColor(127, 105, 82,0.3f)));
        }
    }
